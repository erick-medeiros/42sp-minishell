...
/*   Updated: 2022/11/15 14:19:12 by eandre-f         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	executor(t_minishell *minishell)
{
	...
	sync_tree_execution(minishell->root, minishell->pipeline,
			...
			t_tree *parent, t_tree *root)
{
	...
	command_search(cmd, &minishell->env_list);
	if (cmd->isbuiltin && !minishell->pipeline)
...
void	sync_tree_execution(t_tree *root, t_bool pipeline, int *exit_status)
{
	t_cmd	*cmd;
	...
	if (!root)
		return ;
	if (root->left)
		sync_tree_execution(root->left, pipeline, exit_status);
	if (root->right)
		sync_tree_execution(root->right, pipeline, exit_status);
	if (root->type == TREE_TYPE_CMD)
	{
		cmd = (t_cmd *) root->content;
		if (pipeline || cmd->isbuiltin == FALSE)
		{
